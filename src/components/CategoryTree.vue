<template>
  <div class="category-tree">
    ...
    <div v-if="showAdd" class="add-form">
      <input v-model="newName" class="add-input" placeholder="分类名称" @keyup.enter="handleAdd" />
      <button class="btn-confirm" @click="handleAdd">确定</button>
    </div>
    <div class="tree-content">
      <div class="tree-item all" :class="{ active: selectedId === null }" @click="$emit('select', null)">
        📁 全部
      </div>
      <CategoryNodeItem v-for="node in categories" :key="node.id" :node="node" :depth="0" :selected-id="selectedId"
        @select="$emit('select', $event)" @delete="handleDelete" />
    </div>
  </div>
</template>
