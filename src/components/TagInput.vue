<template>
  <div class="tag-input">
    <div class="tags-area">
      <span v-for="tag in modelValue" :key="tag.id" class="tag-pill"
      ...
      <input ref="inputRef" v-model="query" class="tag-text-input" :placeholder="modelValue.length ? '' : '+ 添加标签'"
        @input="onInput" @keydown.enter.prevent="handleEnter" @keydown.backspace="handleBackspace"
        @blur="showSuggestions = false" @focus="onInput" />
    </div>
    <div v-if="showSuggestions && suggestions.length" class="suggestions">
      <div v-for="tag in suggestions" :key="tag.id" class="suggestion-item" @mousedown.prevent="addTag(tag)">
